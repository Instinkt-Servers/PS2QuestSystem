...
MODULE.Class = "TraWepNewtonLauncher"
MODULE.Name = "Boom sie alle weg!"
MODULE.Description = [[
Gehe in den Traitorshop
und kaufe einen
Newton Launcher %d mal
]]
...
if ( SERVER ) then
	...
	MODULE.DBParameters["NewtonLauncherBought"] = 0
	...
	MODULE.Hooks["TTTOrderedEquipment"] = function(ply, equip, is_item)
		...
		if IsValid(ply) then
			...
			// Check quest status
			if TTTQuests.HasPlayerQuest(ply, "TraWepNewtonLauncher") && !TTTQuests.IsQuestComplete(ply, "TraWepNewtonLauncher") then

				// A player must be detective
				if ply:IsTraitor() then
					
					// Check equipment type
					if ( equip == "weapon_ttt_push" ) then

						// Select a row from table
						local row = sql.MySQLQuery("SELECT NewtonLauncherBought FROM TTTQuests_TraWepNewtonLauncher WHERE SteamID = \"%s\"", ply:SteamID() )

						if row then // Just make sure the row isn't nil
							
							// Get current progress from the row
							local current = row[1].NewtonLauncherBought

							// Check our condition
							if ( current + 1 >= TTTQuests.Quests["TraWepNewtonLauncher"].TargetValue ) then

								// Call the hook
								hook.Run("TTTQuests_QuestComplete", ply, "TraWepNewtonLauncher")
							end

							// Write to new progress to a database
							sql.MySQLQuery("UPDATE TTTQuests_TraWepNewtonLauncher SET NewtonLauncherBought=%d WHERE SteamID = \"%s\"", current + 1, ply:SteamID() )
						end
					end
				end
			end
