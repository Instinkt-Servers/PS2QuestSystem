...
MODULE.Class = "Wepkill_ttt_sg552"
MODULE.Name = "Standard Scar Killer"
MODULE.Description = "Töte %d Spieler mit der Scar"
...
if ( SERVER ) then
	...
	MODULE.DBParameters["KilledWithStndSG552"] = 0
	...
	MODULE.Hooks["DoPlayerDeath"] = function(victim, attacker, dmginfo)
		if ( #player.GetAll() >= TTTQuests.Config.MinPlayers ) then
			...
			if IsValid(victim)
				...
				// Check quest status
				if TTTQuests.HasPlayerQuest(attacker, "Wepkill_ttt_sg552") && !TTTQuests.IsQuestComplete(attacker, "Wepkill_ttt_sg552") then
					// Get attacker current weapon
					local weapon = dmginfo:GetAttacker():GetActiveWeapon()
					// Check weapon class
--					if weapon:GetClass() == string.StartWith(weapon:GetClass(), "m9k_awp_") then
					if weapon:GetClass() == "m9k_scar" && !TTTQuests.IsRDM(victim, attacker) then
						// Select a row from table
						local row = sql.MySQLQuery("SELECT KilledWithStndSG552 FROM TTTQuests_Wepkill_ttt_sg552 WHERE SteamID = \"%s\"", attacker:SteamID() )
						if row then // Just make sure the row isn't nil
							
							// Get current progress from the row
							local currentKills = row[1].KilledWithStndSG552

							// Check our condition
							if ( currentKills + 1 >= TTTQuests.Quests["Wepkill_ttt_sg552"].TargetValue ) then

								// Call the hook
								hook.Run("TTTQuests_QuestComplete", attacker, "Wepkill_ttt_sg552")
							end

							// Write to new progress to a database
							sql.MySQLQuery("UPDATE TTTQuests_Wepkill_ttt_sg552 SET KilledWithStndSG552=%d WHERE SteamID = \"%s\"", currentKills + 1, attacker:SteamID() )
						end
					end
				end
			end
