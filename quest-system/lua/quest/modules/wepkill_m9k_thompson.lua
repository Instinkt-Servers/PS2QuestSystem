...
MODULE.Class = "Wepkill_m9k_thompson"
MODULE.Name = "Thompson Beglücker"
MODULE.Description = "Töte %d Spieler mit der Thompson"
...
if ( SERVER ) then
	...
	MODULE.DBParameters["KilledWithThomspon"] = 0
	...
	MODULE.Hooks["DoPlayerDeath"] = function(victim, attacker, dmginfo)
		if ( #player.GetAll() >= TTTQuests.Config.MinPlayers ) then
			...
			if IsValid(victim)
				...
				// Check quest status
				if TTTQuests.HasPlayerQuest(attacker, "Wepkill_m9k_thompson") && !TTTQuests.IsQuestComplete(attacker, "Wepkill_m9k_thompson") then
...
--					if weapon:GetClass() == string.StartWith(weapon:GetClass(), "m9k_awp_") then
					if weapon:GetClass() == "m9k_contender" && !TTTQuests.IsRDM(victim, attacker) then
						...
						local row = sql.MySQLQuery("SELECT KilledWithThomspon FROM TTTQuests_Wepkill_m9k_thompson WHERE SteamID = \"%s\"", attacker:SteamID() )
						if row then // Just make sure the row isn't nil
							
							// Get current progress from the row
							local currentKills = row[1].KilledWithThomspon

							// Check our condition
							if ( currentKills + 1 >= TTTQuests.Quests["Wepkill_m9k_thompson"].TargetValue ) then

								// Call the hook
								hook.Run("TTTQuests_QuestComplete", attacker, "Wepkill_m9k_thompson")
							end

							// Write to new progress to a database
							sql.MySQLQuery("UPDATE TTTQuests_Wepkill_m9k_thompson SET KilledWithThomspon=%d WHERE SteamID = \"%s\"", currentKills + 1, attacker:SteamID() )
						end
					end
				end
			end
