...
MODULE.Class = "Wepkill_zm_improvised"
MODULE.Name = "Crowbar King"
MODULE.Description = "Töte %d Spieler mit der Crowbar"
MODULE.TargetValue = 20 // This is killed traitors count to finish the quest
...
MODULE.Reward = 100 // Just define it here. It'll be easily edited laterr
...
if ( SERVER ) then
	...
	MODULE.DBParameters["KilledWithCrowbar"] = 0
	...
	MODULE.Hooks["DoPlayerDeath"] = function(victim, attacker, dmginfo)
		if ( #player.GetAll() >= TTTQuests.Config.MinPlayers ) then
			...
			if IsValid(victim)
				...
				// Check quest status
				if TTTQuests.HasPlayerQuest(attacker, "Wepkill_zm_improvised") && !TTTQuests.IsQuestComplete(attacker, "Wepkill_zm_improvised") then
					// Get attacker current weapon
					local weapon = dmginfo:GetAttacker():GetActiveWeapon()
					// Check weapon class
--					if weapon:GetClass() == string.StartWith(weapon:GetClass(), "m9k_awp_") then
					if weapon:GetClass() == "weapon_zm_improvised" && !TTTQuests.IsRDM(victim, attacker) then
						// Select a row from table
						local row = sql.MySQLQuery("SELECT KilledWithCrowbar FROM TTTQuests_Wepkill_zm_improvised WHERE SteamID = \"%s\"", attacker:SteamID() )
						if row then // Just make sure the row isn't nil
							
							// Get current progress from the row
							local currentKills = row[1].KilledWithCrowbar

							// Check our condition
							if ( currentKills + 1 >= TTTQuests.Quests["Wepkill_zm_improvised"].TargetValue ) then

								// Call the hook
								hook.Run("TTTQuests_QuestComplete", attacker, "Wepkill_zm_improvised")
							end

							// Write to new progress to a database
							sql.MySQLQuery("UPDATE TTTQuests_Wepkill_zm_improvised SET KilledWithCrowbar=%d WHERE SteamID = \"%s\"", currentKills + 1, attacker:SteamID() )
						end
					end
				end
			end
