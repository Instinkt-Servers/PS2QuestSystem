...
MODULE.Class = "Wepkill_zm_rifle"
MODULE.Name = "Scout Poser"
MODULE.Description = "Töte %d Spieler mit der Scout"
...
if ( SERVER ) then
	...
	MODULE.DBParameters["KilledWithZmRifle"] = 0
	...
	MODULE.Hooks["DoPlayerDeath"] = function(victim, attacker, dmginfo)
		if ( #player.GetAll() >= TTTQuests.Config.MinPlayers ) then
			...
			if IsValid(victim)
				...
				// Check quest status
				if TTTQuests.HasPlayerQuest(attacker, "Wepkill_zm_rifle") && !TTTQuests.IsQuestComplete(attacker, "Wepkill_zm_rifle") then
					// Get attacker current weapon
					local weapon = dmginfo:GetAttacker():GetActiveWeapon()
					// Check weapon class
--					if weapon:GetClass() == string.StartWith(weapon:GetClass(), "m9k_awp_") then
					if weapon:GetClass() == "weapon_zm_rifle" && !TTTQuests.IsRDM(victim, attacker) then
						// Select a row from table
						local row = sql.MySQLQuery("SELECT KilledWithZmRifle FROM TTTQuests_Wepkill_zm_rifle WHERE SteamID = \"%s\"", attacker:SteamID() )
						if row then // Just make sure the row isn't nil
							
							// Get current progress from the row
							local currentKills = row[1].KilledWithZmRifle

							// Check our condition
							if ( currentKills + 1 >= TTTQuests.Quests["Wepkill_zm_rifle"].TargetValue ) then

								// Call the hook
								hook.Run("TTTQuests_QuestComplete", attacker, "Wepkill_zm_rifle")
							end

							// Write to new progress to a database
							sql.MySQLQuery("UPDATE TTTQuests_Wepkill_zm_rifle SET KilledWithZmRifle=%d WHERE SteamID = \"%s\"", currentKills + 1, attacker:SteamID() )
						end
					end
				end
			end
