...
MODULE.Class = "Survivalistdb3"
MODULE.Name = "fetter Überlebender"
MODULE.Description = "Überlebe %d Runden"
MODULE.TargetValue = 50 // This is surivived rounds count to finish the quest
...
if ( SERVER ) then
	...
	MODULE.DBParameters["RoundSurvived3"] = 0
		...
		if ( #player.GetAll() >= TTTQuests.Config.MinPlayers ) then
			...
			for _, ply in pairs(player.GetAll()) do
				...
				if IsValid(ply) && !ply:IsSpec() then
					...
					// Check quest status
					if TTTQuests.HasPlayerQuest(ply, "Survivalistdb3") && !TTTQuests.IsQuestComplete(ply, "Survivalistdb3") then

						// Select a row from table
						local row = sql.MySQLQuery("SELECT RoundSurvived3 FROM TTTQuests_Survivalistdb3 WHERE SteamID = \"%s\"", ply:SteamID() )

						if row then // Just make sure the row isn't nil
							
							// Get current progress from the row
							local currentRounds = row[1].RoundSurvived3

							// Check our condition
							if ( currentRounds + 1 >= TTTQuests.Quests["Survivalistdb3"].TargetValue ) then

								// Call the hook
								hook.Run("TTTQuests_QuestComplete", ply, "Survivalistdb3")
							end

							// Write to new progress to a database
							sql.MySQLQuery("UPDATE TTTQuests_Survivalistdb3 SET RoundSurvived3=%d WHERE SteamID = \"%s\"", currentRounds + 1, ply:SteamID() )
						end
					end
				end
			end
