...
MODULE.Class = "Wepkill_m9k_mp40"
MODULE.Name = "Vikhr Zerblaser"
MODULE.Description = "Töte %d Spieler mit der vikhr"
...
if ( SERVER ) then
	...
	MODULE.DBParameters["KilledWithMP40"] = 0
	...
	MODULE.Hooks["DoPlayerDeath"] = function(victim, attacker, dmginfo)
		if ( #player.GetAll() >= TTTQuests.Config.MinPlayers ) then
			...
			if IsValid(victim)
				...
				// Check quest status
				if TTTQuests.HasPlayerQuest(attacker, "Wepkill_m9k_mp40") && !TTTQuests.IsQuestComplete(attacker, "Wepkill_m9k_mp40") then
					// Get attacker current weapon
					local weapon = dmginfo:GetAttacker():GetActiveWeapon()
					// Check weapon class
--					if weapon:GetClass() == string.StartWith(weapon:GetClass(), "m9k_awp_") then
					if weapon:GetClass() == "m9k_vikhr" && !TTTQuests.IsRDM(victim, attacker) then
						// Select a row from table
						local row = sql.MySQLQuery("SELECT KilledWithMP40 FROM TTTQuests_Wepkill_m9k_mp40 WHERE SteamID = \"%s\"", attacker:SteamID() )
						if row then // Just make sure the row isn't nil
							
							// Get current progress from the row
							local currentKills = row[1].KilledWithMP40

							// Check our condition
							if ( currentKills + 1 >= TTTQuests.Quests["Wepkill_m9k_mp40"].TargetValue ) then

								// Call the hook
								hook.Run("TTTQuests_QuestComplete", attacker, "Wepkill_m9k_mp40")
							end

							// Write to new progress to a database
							sql.MySQLQuery("UPDATE TTTQuests_Wepkill_m9k_mp40 SET KilledWithMP40=%d WHERE SteamID = \"%s\"", currentKills + 1, attacker:SteamID() )
						end
					end
				end
			end
